...
$con = conexion();
session_start();
 //pregunta si tu id es igual a 12 = admin entonces te redirige a tu pagina
...
//el operador IN Determina si un valor especificado coincide con algún valor de una subconsulta o una lista
//el operador UNION junta los resultados de dos consultas

$sql = "SELECT f_p,nombre,b.id_r
FROM usuarios_r b
WHERE b.id_r IN (SELECT amigo1 FROM amigos WHERE amigo2 = $id /*id*/ UNION SELECT amigo2 FROM amigos WHERE amigo1 = $id)
AND b.id_r != $id";
    ...
    while($fila = pg_fetch_array($obj)){
        ...
        <td class="ims"><img class="im" src="<?=$fila[0]?>" alt="imagen"></td>
          <td class="us"><?=$fila[1]?></td>
           <td class="f"><a href="ichat.php? t=<?=$fila[2]?>"><img class="ff" src="sobre.png" alt="" width="50"></a></td>
           <td class="id"><?=$fila[2]?></td>
        </tr>
        <?php
        }
    ?>
