...
$id = $_SESSION['c'];
    ...
    $sql = "SELECT f_p, nombre,informacion, a.id_r, id_s FROM usuarios_r as a INNER JOIN soli as b ON a.id_r = b.entrega WHERE recibe = $id"; //Consulta a mi tabla
    ...
    <td class="ims"><img class="im" src="<?=$fila[0]?>" alt="imagen"></td>
      ...
      <td class="info"><?=$fila[2]?></td>
       <td class="f"><a href="aceptar.php? t=<?=$fila[3]?>"><img class="ff" src="controlar.png" alt="" width="50"></a></td>
      <td class="f1"><a href="no.php? t=<?=$fila[4]?>"><img class="ff2" src="cruz.png" alt="" width="50"></a></td> 
     <td class="id"><?=$fila[3]?></td>
     <td class="id"><?=$fila[4]?></td>
    </tr>
    <?php
    }
?>
